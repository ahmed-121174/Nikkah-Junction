...
// Get ID of the profile to reject
$profile_id = (int)$_GET['id'];
$current_user_id = $_SESSION['user_id'];

// Don't allow self-rejections
if($profile_id == $current_user_id) {
    $_SESSION['error_message'] = "You cannot reject your own profile.";
    header("Location: dashboard_user_profiles.php");
    exit();
}
...
if($request_result->num_rows > 0) {
    ...
    if($update_stmt->execute()) {
        $_SESSION['success_message'] = "Profile request has been rejected successfully.";
    } else {
    ...
    if($insert_stmt->execute()) {
        $_SESSION['success_message'] = "Profile has been rejected successfully.";
    } else {
        $_SESSION['error_message'] = "Error rejecting profile: " . $conn->error;
    }
